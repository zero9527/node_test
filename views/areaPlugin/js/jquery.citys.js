                ...
                success:function(data){
                    ...
                    var format = {
                        province:function(){
                            $province.empty();
                            if(!options.required){
                                // $province.append('<p value=""> 请选择 </p>');
                            }
                            for(var i in province){
                                $province.append('<p value="'+(options.valueType=='code'?i:province[i])+'" data-code="'+i+'">'+province[i]+'</p>');
                            }
                            if(options.province){
                        ...
                        city:function(){
                            ...
                            if(!hasCity){
                                ...
                                for(var i in city){
                                    $city.append('<p value="'+(options.valueType=='code'?i:city[i])+'" data-code="'+i+'">'+city[i]+'</p>');
                                }
                                if(options.city){
                        ...
                        area:function(){
                            ...
                            for(var i in area){
                                $area.append('<p value="'+(options.valueType=='code'?i:area[i])+'" data-code="'+i+'">'+area[i]+'</p>');
                            }
                            if(options.area){
                    ...
                    $province.on('click', 'p',function(){
                        options.province = $(this).data('code')||0; //选中节点的区划代码
                        options.city = 0;
                        options.area = 0;
                        updateData();
                        format.city();
                        options.onChange(_api.getInfo());
                        $('.ptext').text($(this).text());
                    });
                    $city.on('click', 'p',function(){
                        options.city = $(this).data('code')||0; //选中节点的区划代码
                        options.area = 0;
                        updateData();
                        format.area();
                        options.onChange(_api.getInfo());
                        $('.ctext').text($(this).text());
                    });
                    $area.on('click', 'p',function(){
                        options.area = $(this).data('code')||0; //选中节点的区划代码
                        options.onChange(_api.getInfo());
                        $('.atext').text($(this).text());
                    });
                    //初始化
                    updateData();
                    format.province();
